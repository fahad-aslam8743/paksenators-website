...
import { GalleryCollectionsCMS } from '../components/cms/GalleryCollectionsCMS';
import { ContactMessagesCMS } from '../components/cms/ContactMessagesCMS';
...
export interface CMSSectionConfig {
  ...
  description: string;
  badge?: (data: { applications: MembershipApplication[]; munApplications: MembershipApplication[]; senators: Senator[]; messages: ContactMessage[] }) => number;
  renderComponent: (props: {
    ...
    munApplications: MembershipApplication[];
    senators: Senator[];
...
export const AdminDashboardView: React.FC = () => {
  const {
    ...
    adminUser, adminAuthLoading, loginAdmin, logoutAdmin, dbLive
  } = useYSP();
  ...
  // Data States — seeded from whatever's already cached from a previous
  // fetch this session so the Overview stats/badges don't flash to 0 on
  // every visit; loadAllData() below still always refreshes them for real.
  const [applications, setApplications] = useState<MembershipApplication[]>(
    () => (getCached<MembershipApplication[]>('/applications') || []).filter(a => a.applicationType !== 'MUN')
  );
  const [munApplications, setMunApplications] = useState<MembershipApplication[]>(
    () => (getCached<MembershipApplication[]>('/applications') || []).filter(a => a.applicationType === 'MUN')
  );
  const [senators, setSenators] = useState<Senator[]>(() => getCached<Senator[]>('/senators') || []);
  const [leadership, setLeadership] = useState<LeadershipMember[]>(() => getCached<LeadershipMember[]>('/leadership') || []);
  const [committees, setCommittees] = useState<Committee[]>(() => getCached<Committee[]>('/committees') || []);
  const [chapters, setChapters] = useState<DistrictChapter[]>(() => getCached<DistrictChapter[]>('/chapters') || []);
  const [events, setEvents] = useState<EventItem[]>(() => getCached<EventItem[]>('/events') || []);
  const [news, setNews] = useState<NewsItem[]>(() => getCached<NewsItem[]>('/news') || []);
  const [messages, setMessages] = useState<ContactMessage[]>(() => getCached<ContactMessage[]>('/contact') || []);
  // Drives the small spinning-refresh-button in the header below.
  ...
  const loadAllData = async () => {
    try {
      ...
      // MunAdminCMS.tsx, fed by its own filtered fetch — munApplications
      // below is a second, MUN-only slice of the same response, kept
      // purely so the MUN Admin tab badge and the Overview page can show
      // its own pending count without duplicating MunAdminCMS's fetch.
      if (appData) {
        setApplications(appData.filter(a => a.applicationType !== 'MUN'));
        setMunApplications(appData.filter(a => a.applicationType === 'MUN'));
      }
  ...
  // Manual "Refresh" button in the header — pulls the very latest data
  // (new applications, new senators, etc.) on demand instead of the admin
  ...
  // Picks up new/changed records automatically — e.g. a new application
  // submitted, or a senator approved from another admin session — while
  // this dashboard is already open, without needing a manual page reload.
  // Fed by the background revalidation sweep in lib/api.ts (periodic +
  // on reconnect/tab-focus).
  useEffect(() => {
    const onRefresh = (e: Event) => {
      const detail = (e as CustomEvent).detail;
      if (!detail) return;
      switch (detail.endpoint) {
        case '/applications':
          setApplications((detail.data as MembershipApplication[]).filter(a => a.applicationType !== 'MUN'));
          setMunApplications((detail.data as MembershipApplication[]).filter(a => a.applicationType === 'MUN'));
          break;
        case '/senators':
          setSenators(detail.data as Senator[]);
          break;
        case '/leadership':
          setLeadership(detail.data as LeadershipMember[]);
          break;
        case '/committees':
          setCommittees(detail.data as Committee[]);
          break;
        case '/chapters':
          setChapters(detail.data as DistrictChapter[]);
          break;
        case '/events':
          setEvents(detail.data as EventItem[]);
          break;
        case '/news':
          setNews(detail.data as NewsItem[]);
          break;
        case '/contact':
          setMessages(detail.data as ContactMessage[]);
          break;
      }
    };
  ...
  const CMS_SECTIONS_CONFIG: CMSSectionConfig[] = [
      ...
      renderComponent: (props) => (
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <button
              onClick={() => props.setActiveTab('MunAdmin')}
              className="text-left bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs space-y-2 hover:border-emerald-300 transition"
            >
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Pending MUN Apps</span>
              <p className="text-2xl font-black text-rose-700">
                {props.munApplications.filter(a => a.status === 'Submitted' || a.status === 'Pending' || a.status === 'Under Review').length}
              </p>
            </button>
          </div>
    ...
    },
    {
      ...
      badge: (d) => d.munApplications.filter(a => a.status === 'Submitted' || a.status === 'Pending' || a.status === 'Under Review').length,
    ...
    },
    {
      ...
      icon: Mail,
      description: 'View and respond to messages submitted via the Contact page',
      badge: (d) => d.messages.filter(m => !m.isRead).length,
      renderComponent: () => <ContactMessagesCMS />
    },
    {
      id: 'Branding',
      ...
      renderComponent: () => (
        <GenericCollectionCMS
          ...
          fields={[
            ...
            { key: 'objectives', label: 'Key Objectives (one per line)', type: 'list', placeholder: 'e.g. Review youth policy proposals' },
      ...
      renderComponent: () => (
        <GenericCollectionCMS
          ...
          fields={[
            ...
            { key: 'presidentName', label: 'District President Name', type: 'text' },
            { key: 'presidentPhotoUrl', label: 'District President Photo', type: 'image', imageFolder: 'youth-senate/chapters' },
    ...
    },
    {
      ...
      icon: Images,
      description: 'Multi-photo albums with a title and description',
      renderComponent: () => <GalleryCollectionsCMS />
    },
    {
      id: 'Resolutions',
      label: 'Resolutions',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-100 via-slate-50 to-emerald-50 text-slate-900 relative">
      ...
      {/* Database connectivity notice — informational only. Screens keep
          showing cached/last-known data the whole time; this just tells
          the admin honestly that saves may be temporarily refused until
          the live Firestore connection is back, instead of leaving them
          to discover it only when a save unexpectedly fails. */}
      {!dbLive && (
        <div className="sticky top-0 z-40 bg-amber-500 text-slate-950 text-xs sm:text-sm font-bold text-center py-2 px-3">
          Reconnecting to the database — you can keep browsing, but new changes may not save until the connection is back.
        </div>
      )}
      ...
      <div className="sticky top-0 z-30 backdrop-blur-xl bg-slate-950/80 text-white border-b border-white/10 px-3.5 sm:px-6 py-3 sm:py-4 flex flex-row justify-between items-center gap-3 shadow-lg shadow-slate-950/20">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          <button
            ...
            className="px-2.5 sm:px-3.5 py-1.5 bg-white/10 hover:bg-white/20 text-white border border-white/20 rounded-xl text-xs font-bold flex items-center gap-1.5 transition backdrop-blur-sm disabled:opacity-60"
          >
      ...
      */}
      <div className="max-w-7xl mx-auto px-4 py-6 sm:py-8 grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 relative">
          ...
          */}
          ...
          <nav ref={cmsNavScrollRef} className="flex gap-2 overflow-x-auto no-scrollbar scroll-smooth [@media(hover:hover)_and_(pointer:fine)]:px-8">
            {CMS_SECTIONS_CONFIG.map(tab => {
              ...
              const badgeCount = tab.badge ? tab.badge({ applications, munApplications, senators, messages }) : 0;
         ...
         * Wrapped per-tab so a crash in one CMS section (e.g. a malformed
         * application record breaking the Applications tab) can never take
         * down the sidebar/shell or any other tab — only that one panel
         * shows an error, with a Try Again button, and the rest of the
         * admin dashboard keeps working. */}
        <div className="lg:col-span-9 space-y-6 sm:space-y-8">
          <ErrorBoundary key={activeSection.id} label={activeSection.label || 'This section'}>
