...
import { Certificate } from '../types/ysp';
...
export const CertificateVerifyView: React.FC = () => {
  ...
  const [result, setResult] = useState<{ found: boolean; certificate?: Certificate; message?: string } | null>(null);
  ...
  const performVerification = async (numToVerify: string) => {
    ...
    try {
      const res = await fetchApi<{ found: boolean; certificate?: Certificate; message?: string }>(
