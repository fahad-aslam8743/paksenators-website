...
export const MembershipApplyView: React.FC = () => {
  ...
  // Pulled live from the Standing Committees the admin panel manages
  // (/api/committees — the same data behind the public Committees page),
  // so this dropdown always reflects whichever committees currently exist
  // — including ones added after this form was last deployed — instead of
  // a hardcoded list that silently goes stale.
  ...
  // Tracks which of the 4 file fields is currently mid-upload, so the
  // submit button can be disabled and the field can show a spinner —
  // uploads are now real network requests (to Cloudinary), not instant
  // local base64 reads, so this state didn't exist before but is needed now.
  ...
  const handleFileUpload = async (
    ...
    // Uploaded to Cloudinary and only the resulting URL is stored on the
    // application — NOT the raw file data. Embedding the actual photo as a
    // base64 string directly on the application record (the previous
    // approach) made Firestore documents balloon past its hard 1MB-per-
    // document limit with just 2-3 photos, which silently failed the
    // entire application write. Storing a short URL instead keeps every
    // application document tiny regardless of how many/large the photos
    // attached to it are.
    setUploadingField(field);
    try {
      const result = await uploadImageToFirebase(file, 'youth-senate/applications');
      setFormData(prev => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.preferredCommittee) {
      showNotification('Please select a preferred Standing Committee.', 'error');
      return;
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-300 space-y-3">
              ...
              {formData.passportPhotoUrl ? (
                ...
                <div className="border-2 border-dashed border-emerald-600/60 bg-emerald-50/50 p-4 rounded-xl flex flex-col items-center justify-center space-y-2 text-center">
                  <Loader2 className="w-6 h-6 text-emerald-700 animate-spin" />
                  <span className="font-bold text-emerald-900 text-xs">Uploading...</span>
                </div>
              ) : (
        ...
        <div className="space-y-4 pt-2 border-t">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-300 space-y-3">
              ...
              ) : uploadingField === 'paymentReceiptUrl' ? (
                <div className="border-2 border-dashed border-amber-500 bg-amber-50/50 p-6 rounded-xl flex flex-col items-center justify-center space-y-2 text-center">
                  <Loader2 className="w-8 h-8 text-amber-600 animate-spin" />
                  <span className="font-bold text-emerald-950 text-xs">Uploading...</span>
                </div>
              ) : (
