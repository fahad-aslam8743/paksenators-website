    ...
    const loadLeadership = async () => {
      try {
        ...
        if (firestoreList && firestoreList.length > 0) {
          setLeadership(firestoreList);
          return;
        }
      } catch (e) {
        console.warn('Firestore load warning, trying API');
      }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {filtered.map(member => (
          <div key={member.id} className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between">
            <div>
              <img src={member.photoUrl} alt={member.name} referrerPolicy="no-referrer" className="w-full h-56 object-cover object-top" />
