...
import { getLeadershipFromFirestore } from '../lib/firebaseStorage';
import { LeadershipMember } from '../types/ysp';

export const FounderChairmanView: React.FC = () => {
  const { navigate } = useYSP();
  const [founder, setFounder] = useState<LeadershipMember | null>(null);

  useEffect(() => {
    const loadFounder = async () => {
      try {
        const firestoreList = await getLeadershipFromFirestore();
        if (firestoreList && firestoreList.length > 0) {
          const fc = firestoreList.find(l => l.category === 'Founder Chairman' || l.name.includes('Irfan Mateen'));
          if (fc) {
            setFounder(fc);
            return;
          }
        ...
        console.warn('Firestore load warning');
      }
    ...
    <div className="max-w-7xl mx-auto px-4 py-12 space-y-12 bg-slate-50 text-slate-900">
      ...
      <div className="bg-emerald-950 text-white rounded-2xl p-8 md:p-12 border-b-4 border-amber-500 flex flex-col md:flex-row gap-8 items-center">
        <img
          ...
          className="w-52 h-52 rounded-2xl object-cover object-top border-4 border-amber-400 shadow-2xl shrink-0 bg-white"
        />
