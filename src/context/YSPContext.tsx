...
import { optimisticSingletonSave } from '../lib/optimistic';
...
interface YSPContextType {
  ...
  // Live Firestore connectivity, polled from /api/health. Screens (mainly
  // the admin panel) use this to show an honest "reconnecting to the
  // database" notice instead of leaving the admin guessing about whether
  // what they're editing is stale or whether a save will succeed. This
  // never blanks the screen — cached/last-known data keeps showing either
  // way; it's purely informational.
  dbLive: boolean;
}
...
export const YSPProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [dbLive, setDbLive] = useState<boolean>(true); // optimistic until proven otherwise
  ...
  const updateSiteSettings = async (newSettings: Partial<SiteSettings>) => {
    // Optimistic: the admin sees the change applied immediately; rolled
    // back automatically if the server/Firestore rejects it.
    await optimisticSingletonSave(
      siteSettings,
      setSiteSettings,
      '/settings',
      newSettings,
      showNotification,
      { success: 'Site settings updated successfully.', failure: 'Failed to update settings' }
    );
  };
  ...
  // Poll server/Firestore health so the UI can show an honest "database
  // reconnecting" state rather than silently mixing stale cache with an
  // implied "everything is fine". Never used to hide or clear data —
  // purely an informational signal.
  useEffect(() => {
    let cancelled = false;
    const checkHealth = async () => {
      try {
        const health = await fetchApi<{ firestoreLive: boolean }>('/health');
        if (!cancelled) setDbLive(!!health?.firestoreLive);
      } catch {
        if (!cancelled) setDbLive(false);
      }
    };
    checkHealth();
