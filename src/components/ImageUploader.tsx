...
export const ImageUploader = forwardRef<ImageUploaderRef, ImageUploaderProps>(({
  ...
  const handleDeletePhoto = async () => {
    ...
    // Deliberately does NOT delete the Storage file here. At this point
    // we don't know whether the caller will actually persist this change
    // (the surrounding CMS form might still be cancelled, or its save
    // might fail) — deleting the file now and having the save fail/be
    // cancelled would leave the still-live Firestore record pointing at
    // a file that no longer exists. Per the "image delete safety" rule,
    // clearing this field is a local/form-only action; the caller (via
    // onDeletePhoto) is responsible for deleting the actual Storage file
    // only after it has confirmed its own save succeeded.
  ...
  const uploadImage = async (): Promise<{ downloadUrl: string; storagePath: string } | null> => {
    ...
    try {
      // Upload the new file to Firebase Storage. Deliberately does NOT
      // delete the old Storage file here: at this point the caller (the
      // CMS form) has not yet saved the record that will point at the new
      // file, so if that save fails afterwards, deleting the old image now
      // would leave the still-live record pointing at a file that no
      // longer exists. Per the "image delete safety" rule, an orphaned old
      // file is a much smaller problem than a broken production record —
      // so cleanup of the old file is left to the caller, to run only
      // after it has confirmed the new record was saved successfully.
