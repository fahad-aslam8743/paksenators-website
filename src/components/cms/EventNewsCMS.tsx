...
export const EventNewsCMS: React.FC = () => {
  ...
  const [eventForm, setEventForm] = useState<Partial<EventItem>>({
    ...
    imageUrl: '/images/ysp_official_logo_1786441197850.jpg',
    ...
    try {
      ...
      if (editingId) {
        await optimisticUpdate(
          events, setEvents, '/events', editingId, payload as EventItem,
          showNotification,
          { success: 'Event updated successfully!', failure: 'Failed to save event.' }
        );
      } else {
        const newEvent = { ...payload, id: `evt-${Date.now()}-${Math.random().toString(36).slice(2, 8)}` } as EventItem;
        await optimisticCreate(
          events, setEvents, '/events', newEvent,
          showNotification,
          { success: 'New event published!', failure: 'Failed to save event.' }
        );
      }
    } catch (e: any) {
  ...
  const handleSaveNews = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await optimisticUpdate(
          news, setNews, '/news', editingId, payload as NewsItem,
          showNotification,
          { success: 'News post updated!', failure: 'Failed to save news.' }
        );
      } else {
        const newArticle = { ...payload, id: `news-${Date.now()}-${Math.random().toString(36).slice(2, 8)}` } as NewsItem;
        await optimisticCreate(
          news, setNews, '/news', newArticle,
          showNotification,
          { success: 'News article published!', failure: 'Failed to save news.' }
        );
      }
    } catch (e: any) {
  ...
  const handleDeleteEvent = async (id: string) => {
    ...
    await optimisticDelete(
      events, setEvents, '/events', id,
      showNotification,
      { success: 'Event removed.', failure: 'Failed to delete event.' }
    );
  };
  ...
  const handleDeleteNews = async (id: string) => {
    ...
    await optimisticDelete(
      news, setNews, '/news', id,
      showNotification,
      { success: 'News article removed.', failure: 'Failed to delete news article.' }
    );
  };
