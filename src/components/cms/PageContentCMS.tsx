...
export const PageContentCMS: React.FC = () => {
  ...
  const handleSavePage = async (e: React.FormEvent) => {
    e.preventDefault();
    setSaving(true);
    // Decide create-vs-update from what's actually known locally (whether
    // this page already exists in the loaded list), not by trying PUT and
    // treating ANY failure as "not found" — a 503 (database reconnecting)
    // or any other transient error must never fall through to a POST and
    // risk creating a duplicate page document.
    const exists = pages.some(p => p.id === activePage.id);
    const ok = exists
      ? await optimisticUpdate(
          pages, setPages, '/pages', activePage.id, activePage,
          showNotification,
          { success: `Page "${activePage.pageName}" saved successfully!`, failure: 'Failed to save page changes.' }
        )
      : await optimisticCreate(
          pages, setPages, '/pages', activePage,
          showNotification,
          { success: `Page "${activePage.pageName}" saved successfully!`, failure: 'Failed to save page changes.' }
        );
    setSaving(false);
