...
export type FieldType = 'text' | 'textarea' | 'number' | 'date' | 'select' | 'image' | 'media' | 'checkbox' | 'list';
...
export const GenericCollectionCMS: React.FC<GenericCollectionCMSProps> = ({
  ...
  const handleOpenAdd = () => {
    ...
    fields.forEach(f => {
      if (!(f.key in initial)) {
        initial[f.key] = f.type === 'checkbox' ? true : f.type === 'number' ? 0 : f.type === 'list' ? [] : '';
      }
    });
    setForm(initial);
    setIsModalOpen(true);
  ...
  const handleOpenEdit = (item: any) => {
    ...
    setForm(item);
    setOriginalForm(item); // snapshot at edit-open time, used to compute a diff on save
    setIsModalOpen(true);
  };
  ...
  /** Shallow diff between what the form looked like when the edit modal
   * opened and what it looks like now. Sending only the changed fields
   * (rather than the whole form object) means a field another admin
   * changed in Firestore in the meantime — which this form never touched
   * — is never clobbered by our stale copy of it. */
  const diffChangedFields = (original: Record<string, any>, current: Record<string, any>): Record<string, any> => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Clean up 'list' fields: strip blank lines and surrounding
      // whitespace now that editing is done, so a stray trailing blank
      // line (kept during typing so it doesn't get eaten mid-edit)
      // doesn't get saved as an empty list entry.
      const listFields = fields.filter(f => f.type === 'list');
      for (const f of listFields) {
        const raw = finalForm[f.key];
        finalForm[f.key] = Array.isArray(raw)
      ...
      if (editingId) {
        // Send only the fields that actually changed — never the whole
        // (possibly stale) form object — so a concurrent edit to some
        // other field by another admin is never overwritten.
        const patch = diffChangedFields(originalForm, finalForm);
        ...
        // Optimistic: apply the patch to the UI immediately...
        setItems(prev => prev.map(it => (it.id === editingId ? { ...it, ...patch } : it)));
        setIsModalOpen(false);
        try {
          const saved = await fetchApi<any>(`${endpoint}/${editingId}`, {
            method: 'PUT',
            body: JSON.stringify(patch)
          });
          // ...server confirmed — reconcile with its response.
          setItems(prev => prev.map(it => (it.id === editingId ? { ...it, ...(saved || patch) } : it)));
          showNotification(`${getItemTitle(finalForm) || title} updated successfully!`, 'success');
        } catch (err: any) {
          // ...server rejected it — restore exactly what was there before.
        ...
        const previousItems = items;
        // Optimistic: show the new item immediately...
        ...
        try {
          ...
          setItems(prev => prev.map(it => (it.id === payload.id ? (saved || payload) : it)));
          showNotification(`${getItemTitle(finalForm) || title} added successfully!`, 'success');
        } catch (err: any) {
          // ...server rejected it — remove the optimistic item again.
          setItems(previousItems);
          showNotification(err.message || `Failed to save ${title}.`, 'error');
        }
      }
    } catch (err: any) {
      // Errors from the upload steps above (before any optimistic UI
      // change was made) — nothing to roll back, just report it.
      showNotification(err.message || `Failed to save ${title}.`, 'error');
    } finally {
      setSaving(false);
    }
            ...
            <form onSubmit={handleSave} className="space-y-4">
              {fields.map(f => (
                <div key={f.key} className="space-y-1">
                  ...
                  {f.type === 'list' && (
                    <textarea
                      // Stored as a string[] under the hood (one entry per
                      // line) — the textarea itself just edits it as
                      // newline-separated text. Blank lines are kept while
                      // typing so a trailing new line doesn't get eaten
                      // mid-edit; they're stripped out on save instead.
                      value={Array.isArray(form[f.key]) ? form[f.key].join('\n') : (form[f.key] ?? '')}
                      onChange={e => handleChange(f.key, e.target.value.split('\n'))}
                      required={f.required}
