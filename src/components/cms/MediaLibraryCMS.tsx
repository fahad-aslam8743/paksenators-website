...
export const MediaLibraryCMS: React.FC = () => {
  ...
  const handleDelete = async (item: MediaItem) => {
    ...
    // Optimistic — disappears immediately, restored automatically if the
    // server/Firestore rejects the delete.
    await optimisticDelete(
      mediaList, setMediaList, '/media', item.id,
      showNotification,
      { success: 'Media asset deleted.', failure: 'Failed to delete media asset.' }
    );
  };
      ...
      <div className="p-6 bg-slate-50 border border-slate-200 rounded-2xl space-y-4">
        ...
        <ImageUploader
          ...
          onUploadSuccess={async (url, path) => {
            // The actual file is already uploaded and real by this point —
            // only the Firestore metadata write is left, so that part can
            // safely be optimistic (unlike the upload itself, which is the
            // real bottleneck and must actually finish first).
            ...
            await optimisticCreate(
              mediaList, setMediaList, '/media', newMedia,
              showNotification,
              { success: 'New asset saved to Central Media Library!', failure: 'Uploaded, but failed to save it to the media library record.' }
            );
          }}
        />
      </div>
