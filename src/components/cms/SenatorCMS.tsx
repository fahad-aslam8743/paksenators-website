...
export const SenatorCMS: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        // Optimistic: the edit shows immediately; rolled back automatically
        // if the server/Firestore rejects it.
        await optimisticUpdate(
          senators, setSenators, '/senators', editingId, payload as Senator,
          showNotification,
          { success: `Senator ${form.name} updated!`, failure: 'Failed to save senator details.' }
        );
      } else {
        const newSenator = { ...payload, id: `sen-${Date.now()}-${Math.random().toString(36).slice(2, 8)}` } as Senator;
        await optimisticCreate(
          senators, setSenators, '/senators', newSenator,
          showNotification,
          { success: `New Senator ${form.name} added!`, failure: 'Failed to save senator details.' }
        );
      }
    } catch (e: any) {
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    // Optimistic: disappears immediately, restored automatically on failure.
    await optimisticDelete(
      senators, setSenators, '/senators', id,
      showNotification,
      { success: `Senator ${name} removed.`, failure: 'Failed to delete senator.' }
    );
  };
          ...
          <div key={senator.id} className="bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-4 shadow-xs relative">
            <div className="flex items-start gap-4">
              <img
                ...
                alt={senator.name}
                className="w-16 h-16 rounded-xl object-cover object-[center_15%] border-2 border-slate-200 shadow-xs shrink-0"
              />
