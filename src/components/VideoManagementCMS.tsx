...
import { 
  ...
  deleteStorageFile,
  permanentlyDeleteVideoFromYouTube
...
export const VideoManagementCMS: React.FC<VideoManagementCMSProps> = ({ onNotification }) => {
  ...
  // Delete-from-website Confirmation Modal (Firestore metadata only — YouTube is untouched)
  const [deletingVideo, setDeletingVideo] = useState<VideoItem | null>(null);

  // Permanent YouTube Deletion — separate, explicit, strongly-confirmed action
  const [youtubeDeleteTarget, setYoutubeDeleteTarget] = useState<VideoItem | null>(null);
  const [youtubeDeleteConfirmText, setYoutubeDeleteConfirmText] = useState<string>('');
  const [deletingFromYoutube, setDeletingFromYoutube] = useState<boolean>(false);
  const YOUTUBE_DELETE_CONFIRM_PHRASE = 'PERMANENTLY DELETE FROM YOUTUBE';
  ...
  const handleSaveForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const previousThumbnailStoragePath = formData.thumbnailStoragePath || '';
      ...
      // Save the record FIRST. Only once Firestore has actually confirmed
      // the new video/thumbnail references are saved do we delete the old
      // files — never before. If this save throws, the old video and old
      // thumbnail are still intact and the record still points at them,
      ...
      // Record save confirmed — now it's safe to clean up the old
      // THUMBNAIL image (Firebase Storage). Best-effort: a cleanup
      // failure just leaves a harmless orphaned file, logged but not
      // surfaced as a save error since the actual save already succeeded.
      //
      // NOTE: the previous video file itself is deliberately NOT deleted
      // here, even when replaced. YouTube is the permanent video archive —
      // replacing a video on the site must not delete the old file from
      // YouTube. If the old YouTube video should really be removed, that
      // is a separate, explicit "Permanently delete from YouTube" action
      // an admin can take on it directly.
      if (previousThumbnailStoragePath && previousThumbnailStoragePath !== finalThumbnailStoragePath) {
        deleteStorageFile(previousThumbnailStoragePath).catch(e => console.warn('Could not remove old thumbnail file:', e));
      }
  ...
  const handleToggleStatus = async (video: VideoItem) => {
    ...
    // Optimistic: flip the status in the UI immediately...
    setVideos(prev => prev.map(v => (v.id === video.id ? { ...v, status: newStatus } : v)));
    try {
      await toggleVideoStatusInFirestore(video.id, newStatus);
      // ...Firestore confirmed — keep the optimistic state (no re-fetch needed).
      if (onNotification) {
        onNotification(`Video status changed to ${newStatus.toUpperCase()}`);
      }
    } catch (e: any) {
      // ...Firestore rejected it — restore exactly what was there before.
      setVideos(previousVideos);
      if (onNotification) onNotification('Failed to change status. Please try again.', 'error');
    }
  };

  /** Removes the video from the WEBSITE only (Firestore metadata document).
   * The file on YouTube is never touched by this action. */
  const handleDeleteConfirm = async () => {
    ...
    try {
      await deleteVideoFromFirestore(target.id, target.thumbnailStoragePath);
      if (onNotification) onNotification('Video removed from the website. It remains on YouTube.');
    } catch (e: any) {
      if (onNotification) onNotification('Failed to remove video.', 'error');
      setVideos(previousVideos); // roll back
    }
  };

  /** Separate, explicit, irreversible action: permanently deletes the
   * underlying file from the YouTube channel. Never triggered as a side
   * effect of any other action — only from its own strongly-confirmed
   * modal below. */
  const handlePermanentYouTubeDelete = async () => {
    if (!youtubeDeleteTarget) return;
    if (youtubeDeleteConfirmText.trim() !== YOUTUBE_DELETE_CONFIRM_PHRASE) return;
    setDeletingFromYoutube(true);
    try {
      await permanentlyDeleteVideoFromYouTube(youtubeDeleteTarget.storagePath);
      if (onNotification) onNotification('Video permanently deleted from YouTube.');
      setYoutubeDeleteTarget(null);
      setYoutubeDeleteConfirmText('');
    } catch (e: any) {
      if (onNotification) onNotification(e.message || 'Failed to delete video from YouTube.', 'error');
    } finally {
      setDeletingFromYoutube(false);
    }
  };
  ...
  return (
    <div className="space-y-6 text-slate-900">
                    ...
                    <td className="py-3 px-4 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        <button
                          ...
                          title="Remove From Website (keeps the file on YouTube)"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
                        {vid.storagePath?.startsWith('youtube:') && (
                          <button
                            onClick={() => { setYoutubeDeleteTarget(vid); setYoutubeDeleteConfirmText(''); }}
                            className="p-1.5 bg-slate-800 hover:bg-red-700 text-red-300 hover:text-white rounded-lg transition-colors"
                            title="Permanently Delete From YouTube"
                          >
      ...
      {deletingVideo && (
        <div className="fixed inset-0 bg-slate-950/80 z-[100] flex items-start justify-center p-4 pt-20 sm:pt-24 overflow-y-auto">
          <div className="max-w-md w-full bg-slate-900 text-white rounded-2xl p-6 border border-slate-800 space-y-4 text-center">
            ...
            <h3 className="text-base font-black">Remove Video From Website</h3>
            <p className="text-xs text-slate-300">
              Remove <strong className="text-amber-400">"{deletingVideo.title}"</strong> from the website?
              This deletes only its Firestore record — <strong className="text-emerald-400">the video file stays on YouTube</strong> and
              can be re-added later. To permanently delete the YouTube file itself, use "Delete from YouTube" instead.
            </p>
      ...
      {/* PERMANENT YOUTUBE DELETION MODAL — separate, explicit, strongly-confirmed */}
      {youtubeDeleteTarget && (
        <div className="fixed inset-0 bg-slate-950/80 z-[100] flex items-start justify-center p-4 pt-20 sm:pt-24 overflow-y-auto">
          <div className="max-w-md w-full bg-slate-900 text-white rounded-2xl p-6 border border-red-700/60 space-y-4 text-center">
            <div className="w-12 h-12 bg-red-950 rounded-full border border-red-700/80 flex items-center justify-center mx-auto text-red-400">
              <XCircle className="w-6 h-6" />
            </div>
            <h3 className="text-base font-black text-red-400">Permanently Delete From YouTube</h3>
            <p className="text-xs text-slate-300">
              This will <strong className="text-red-400">permanently delete</strong> the underlying video file for
              <strong className="text-amber-400"> "{youtubeDeleteTarget.title}"</strong> from the YouTube channel itself.
              This cannot be undone and is completely separate from removing it from the website.
            </p>
            <p className="text-[11px] text-slate-400">
              Type <strong className="text-white font-mono">{YOUTUBE_DELETE_CONFIRM_PHRASE}</strong> to confirm:
            </p>
            <input
              type="text"
              value={youtubeDeleteConfirmText}
              onChange={e => setYoutubeDeleteConfirmText(e.target.value)}
              placeholder={YOUTUBE_DELETE_CONFIRM_PHRASE}
              className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-xs text-white text-center font-mono"
            />
            <div className="flex gap-3 justify-center pt-2">
              <button
                onClick={() => { setYoutubeDeleteTarget(null); setYoutubeDeleteConfirmText(''); }}
                className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold rounded-xl"
              >
                Cancel
              </button>
              <button
                onClick={handlePermanentYouTubeDelete}
                disabled={youtubeDeleteConfirmText.trim() !== YOUTUBE_DELETE_CONFIRM_PHRASE || deletingFromYoutube}
                className="px-5 py-2 bg-red-600 hover:bg-red-700 disabled:opacity-40 disabled:cursor-not-allowed text-white text-xs font-black uppercase rounded-xl shadow-lg flex items-center gap-2"
              >
                {deletingFromYoutube ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : null}
                Delete Permanently
              </button>
            </div>
          </div>
