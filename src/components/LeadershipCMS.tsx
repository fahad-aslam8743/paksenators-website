...
export const LeadershipCMS: React.FC<Props> = ({ onDataUpdated }) => {
  ...
  const handleSaveMember = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const idToSave = editingId || `lead-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
      ...
      // Optimistic: the profile shows/updates immediately; rolled back
      // automatically if the server/Firestore rejects it. Firestore (via
      // this server's API) remains the only source of truth — nothing is
      // ever written to disk.
      const ok = editingId
        ? await optimisticUpdate(
            leadershipList, setLeadershipList, '/leadership', editingId, payload,
            showNotification,
            { success: `Leadership profile for "${payload.name}" saved successfully!`, failure: 'Failed to save profile.' }
          )
        : await optimisticCreate(
            leadershipList, setLeadershipList, '/leadership', payload,
            showNotification,
            { success: `Leadership profile for "${payload.name}" saved successfully!`, failure: 'Failed to save profile.' }
          );

      if (ok && onDataUpdated) onDataUpdated();
      if (!ok) setUploadError('Failed to save profile.');
  ...
  const handleDeleteMember = async (member: LeadershipMember) => {
    ...
    // Optimistic removal — disappears immediately, restored on failure.
    const previousList = leadershipList;
    setLeadershipList(prev => prev.filter(m => m.id !== member.id));
    try {
      await fetchApi(`/leadership/${member.id}`, { method: 'DELETE' });
      // Only once the record is actually gone do we clean up its photo, so
      // a failed record delete can never leave a still-listed profile with
      // a missing image.
      if (member.photoStoragePath) {
        deleteImageFromFirebase(member.photoStoragePath).catch(e => console.warn('Could not remove photo file:', e));
      }
      showNotification(`Deleted profile "${member.name}".`);
      if (onDataUpdated) onDataUpdated();
    } catch (err: any) {
      setLeadershipList(previousList);
            ...
            >
              <div className="p-5 space-y-4">
                <div className="flex gap-4 items-start">
                  <img
                    src={member.photoUrl || '/images/irfan_mateen_original_1786443966305.jpg'}
                    alt={member.name}
                    className="w-20 h-24 object-cover object-[center_15%] rounded-xl border-2 border-emerald-800 shadow bg-slate-100 shrink-0"
                  />
